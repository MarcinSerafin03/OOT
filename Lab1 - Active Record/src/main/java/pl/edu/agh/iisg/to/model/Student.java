...
public class Student {
    ...
    public Map<Course, Float> createReport() {
        ...
        String sql = "SELECT * FROM grade WHERE student_id = (?) ORDER BY course_id";
        Object[] args = {id};
        try (ResultSet rs = QueryExecutor.read(sql, args)) {
            while (rs.next()) {
                int courseId = rs.getInt("course_id");
                float grade = rs.getFloat("grade");
                Course course = Course.findById(courseId).orElseThrow();
                if (report.containsKey(course)) {
                    report.put(course, (report.get(course) + grade) / 2);
                } else {
                    report.put(course, grade);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
